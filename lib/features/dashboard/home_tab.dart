...
FutureProvider.autoDispose<Map<String, dynamic>>((ref) async {
  // fetchTodayData() NO LONGER requests permissions internally.
  // It only reads data if permissions are already granted.
  return await healthService.fetchTodayData();
});

// Tracks whether Health Connect permissions have been granted this session.
// Separate from the data fetch so the banner can react independently.
final hcPermissionsProvider = StateProvider<bool>((ref) => false);
...
class _HomeTabState extends ConsumerState<HomeTab> {
  bool _vitalsSaved = false;   // FIXED: guard so vitals only save once per data load

  @override
  void initState() {
    super.initState();
    // Check permissions silently on load — no dialog, no crash
    _checkPermissions();
  }
  ...
  Future<void> _checkPermissions() async {
    ...
    if (mounted) {
      ref.read(hcPermissionsProvider.notifier).state = granted;
    }
  }

  Future<void> _requestPermissions() async {
    // This is called from a button tap — correct Android context
  ...
  Widget build(BuildContext context) {
    ...
    final hasPerms   = ref.watch(hcPermissionsProvider);
        ...
        child: RefreshIndicator(
          ...
          child: healthAsync.when(
            ...
            error: (e, _) => _buildContent(
                context, ref, profile, {}, hasPerms: hasPerms, error: e.toString()),
            data: (data) {
              // FIXED: Save vitals only once per data load, not on every rebuild.
              // Previously _saveToBackend() was called inside _buildContent()
              // which runs on every Flutter rebuild — creating duplicate records.
              if (!_vitalsSaved) {
                _vitalsSaved = true;
                _saveToBackend(ref, data);
              }
              return _buildContent(context, ref, profile, data, hasPerms: hasPerms);
            },
          ),
        ),
      ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          // FIXED: Show permission banner when Health Connect access not granted
          if (!hasPerms) _permissionBanner(),
      ...
      child: Row(children: [
        ...
        const Expanded(
          child: Column(
            ...
            children: [
              Text('Health Connect access needed',
                  style: TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w600,
                      color: Color(0xFF633806))),
              Text('Grant access to see your real vitals',
                  style: TextStyle(fontSize: 11, color: AppColors.warning)),
            ],
          ),
        ),
        GestureDetector(
          onTap: _requestPermissions,   // Called from UI — correct Android context
          child: Container(
  ...
  Map<String, String>? _getCriticalAlert(double hr, double spo2, double temp) {
    if (spo2 > 0 && spo2 < 90) {
      return {
        'parameter': 'SpO₂',
  ...
  Future<void> _saveToBackend(WidgetRef ref, Map<String, dynamic> data) async {
    try {
            ...
            child: Row(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    // FIXED: Shows real connection status, not hardcoded "Live"
                    Container(
                      padding: const EdgeInsets.symmetric(
      ...
      child: GridView.count(
        ...
        children: [
          ...
          MetricCard(
            ...
            statusColor:
            steps <= 0 ? AppColors.textHint : AppColors.blue,
            statusBg:
            steps <= 0 ? AppColors.surface : AppColors.blueLight,
          ...
          MetricCard(
            ...
            status: cal <= 0
                ...
                : 'Keep moving',
            statusColor:
            cal <= 0 ? AppColors.textHint : AppColors.warning,
            statusBg:
            cal <= 0 ? AppColors.surface : AppColors.warningLight,
              ...
              data: (lab) {
                ...
                return Column(
                  children: rows.asMap().entries.map((e) {
                    ...
                    final row = e.value;
                    return _labRow(row['label'], row['value'], isLast);
                  }).toList(),
                );
              },
            ),
