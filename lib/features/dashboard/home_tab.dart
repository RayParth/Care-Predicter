...
class HomeTab extends ConsumerWidget {
  ...
  Widget _buildContent(
    ...
    // 'connected' is set by HealthConnectService — true only when real data exists
    final connected = (data['connected'] as bool?) ?? false;

    // Score is 0 when no data — calculateHealthScore handles this
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          _buildHeader(context, ref, profile, score, sys, dia, connected),
  ...
  // ── Header ────────────────────────────────────────────────────────────────
  //
  // FIXED: badge label now reflects real connection status.
  // FIXED: score card shows correct state when score is 0.
  //
  ...
  Widget _buildHeader(
      ...
      double sys,
      double dia,
      bool connected,   // ← new param
      ) {
    // Determine score label based on actual score value
    final String scoreLabel;
    final Color  scoreLabelColor;

    if (score == 0) {
      scoreLabel      = 'No health data yet';
      scoreLabelColor = AppColors.textHint;
    } else if (score >= 85) {
      scoreLabel      = 'Excellent';
      scoreLabelColor = AppColors.tealMid;
    } else if (score >= 70) {
      scoreLabel      = 'Good condition';
      scoreLabelColor = AppColors.tealMid;
    } else if (score >= 55) {
      scoreLabel      = 'Fair — monitor closely';
      scoreLabelColor = AppColors.warning;
    } else {
      scoreLabel      = 'Needs attention';
      scoreLabelColor = AppColors.danger;
    }
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    // Score number — shows '--' when no data instead of 100
                    Text(
                      score == 0 ? '--' : '$score',
                      style: const TextStyle(
                          fontSize: 36,
                    ...
                    // Score label — reflects actual state
                    Text(
                      scoreLabel,
                      style: TextStyle(
                          fontSize: 12, color: scoreLabelColor),
                    ),
                  ],
                ),
                Column(
                  ...
                  children: [
                    // FIXED BADGE: shows real connection status
                    Container(
                      padding: const EdgeInsets.symmetric(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            // Shows real status — not hardcoded 'Live'
                            connected
                                ? 'Health Connect: Live'
  ...
  Future<void> _saveToBackend(
    ...
    try {
      ...
      print('Backend vitals save error: $e');
    }
  }
              ...
              data: (lab) {
                ...
                void addRow(String key, String label, String? unit) {
                  ...
                  if (val != null && val is num && val > 0) {
                    rows.add({
                      'label': label,
                      'value': '${val.toStringAsFixed(val % 1 == 0 ? 0 : 1)}${unit != null ? ' $unit' : ''}',
                    });
                  }
                }
      ...
      child: Row(
        ...
        children: [
          ...
          Row(children: [
            ...
            const StatusBadge(label: 'Normal', type: BadgeType.normal),
          ]),
        ],
      ),
    );
